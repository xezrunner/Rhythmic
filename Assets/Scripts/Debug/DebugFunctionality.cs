...
public static class DebugFunctionality
{
    ...
    public static void HandleSongOffsetting()
    {
        ...
        if (!Keyboard.current.leftCtrlKey.isPressed && Keyboard.current.numpad8Key.wasPressedThisFrame) // DOESN'T WORK FOR SOME REASON
        {
            ...
            DEBUG_OffsetSong(1);
        else if ((Keyboard.current.leftAltKey.isPressed && Keyboard.current.numpad9Key.isPressed) || (AmpPlayerInputHandler.Instance && AmpPlayerInputHandler.IsActive && Gamepad.current.dpad.up.isPressed))
